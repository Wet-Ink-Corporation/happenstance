...
item: HS-S0142
stage: discover
created: 2026-08-17T06:08:11.503Z
updated: 2026-08-17T06:08:11.503Z
template_sig: 86ce4036
rendered_sig: 3c0a597e
---

# Discover — A clause id a page cites either resolves or fails the gate
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `checked-documentation-surface/_storymap.md` | Parse the clause ids a narrative page cites with the hard-error-on-unreadable posture, resolve each through `clause_ids`, and report every dangling id as a problem naming the page — a page citing a real id passes, one citing a nonexistent id fails. |
| Project acceptance criteria this slice traces to | `checked-documentation-surface/_storymap.md` | AC-007 — the spec's AC-### must map onto these, not restate them |
...
| Dependency edges | `checked-documentation-surface/_storymap.md` | `narrative-checker-mounted-with-pinned-path`, `spec-trace-clause-id-accessor` |
...
Parse the clause ids a narrative page cites with the hard-error-on-unreadable posture, resolve each through `clause_ids`, and report every dangling id as a problem naming the page — a page citing a real id passes, one citing a nonexistent id fails.
...
A citation check that verifies the id matches `[A-Z]{2}-\d+` but never that the clause exists, so `ES-99` resolves happily. A rule no page can fail.
