...
item: HS-S0139
stage: discover
created: 2026-08-17T06:04:25.468Z
updated: 2026-08-17T06:04:25.468Z
template_sig: 86ce4036
rendered_sig: b94948fa
---

# Discover — No fence opts out of the check unnoticed
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `checked-documentation-surface/_storymap.md` | Walk every fence in the tree: reject untagged fences, enumerate info-string parts exhaustively so an unrecognised part is a hard error, reject `ignore`-class fences unless they appear on an enumerated allowance `const`, and sweep that list in reverse so a stale allowance is itself a problem. |
| Project acceptance criteria this slice traces to | `checked-documentation-surface/_storymap.md` | AC-004 — the spec's AC-### must map onto these, not restate them |
...
| Dependency edges | `checked-documentation-surface/_storymap.md` | `narrative-checker-mounted-with-pinned-path` |
...
Walk every fence in the tree: reject untagged fences, enumerate info-string parts exhaustively so an unrecognised part is a hard error, reject `ignore`-class fences unless they appear on an enumerated allowance `const`, and sweep that list in reverse so a stale allowance is itself a problem.
...
behaviour, the AC-### enumeration mapping onto AC-004, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

A fence walk that recognises the info-strings it knows and silently ignores the rest, so a novel opt-out spelling passes unnoticed. `ignore` has already returned by the back door once in this repository's own testkit doctests.
