...
item: HS-S0137
stage: discover
created: 2026-08-17T06:02:01.465Z
updated: 2026-08-17T06:02:01.465Z
template_sig: 86ce4036
rendered_sig: 3d9439d5
---

# Discover — A prose-only change selects the package that compiles it
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `checked-documentation-surface/_storymap.md` | Extend `xtask/src/affected.rs`'s selection arm so a change confined to the narrative tree selects the `xtask` package, with unit tests in both directions, so the story-grain gate does not compile nothing on the very PR that broke a page. |
| Project acceptance criteria this slice traces to | `checked-documentation-surface/_storymap.md` | AC-009 — the spec's AC-### must map onto these, not restate them |
...
Extend `xtask/src/affected.rs`'s selection arm so a change confined to the narrative tree selects the `xtask` package, with unit tests in both directions, so the story-grain gate does not compile nothing on the very PR that broke a page.
...
A selection arm that maps the narrative tree to NO package, so `cargo xtask affected` compiles nothing on the very PR that broke a page. Green gate, broken docs — the failure mode `affected.rs`'s INERT list already has for `docs/`.
