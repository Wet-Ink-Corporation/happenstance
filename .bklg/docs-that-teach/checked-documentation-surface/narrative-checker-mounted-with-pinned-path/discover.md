...
item: HS-S0138
stage: discover
created: 2026-08-17T06:03:12.972Z
updated: 2026-08-17T06:03:12.972Z
template_sig: 86ce4036
rendered_sig: e2681c0d
---

# Discover — The checker is mounted, the tree is pinned by constant, and no page is an orphan
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `checked-documentation-surface/_storymap.md` | Add the bin-crate checker module holding the tree and harness paths as `const`s, erroring by name on a missing tree and `bail!`ing on an empty one, cross-checking page-to-harness registration in both directions, and mount it as its own `REQUIRED` step, subcommand, help line and `lint_steps` member. |
| Project acceptance criteria this slice traces to | `checked-documentation-surface/_storymap.md` | AC-001, AC-005 — the spec's AC-### must map onto these, not restate them |
...
| Dependency edges | `checked-documentation-surface/_storymap.md` | `pinned-narrative-tree-and-compiling-step` |
...
Add the bin-crate checker module holding the tree and harness paths as `const`s, erroring by name on a missing tree and `bail!`ing on an empty one, cross-checking page-to-harness registration in both directions, and mount it as its own `REQUIRED` step, subcommand, help line and `lint_steps` member.
...
behaviour, the AC-### enumeration mapping onto AC-001, AC-005, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

A checker that exists as a function nobody calls, or one whose tree path is a string literal duplicated in three files so moving the tree silently disables it. Compiles, tests pass, checks nothing.
