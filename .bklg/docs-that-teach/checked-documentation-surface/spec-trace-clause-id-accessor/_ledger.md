...
- id: AC-001
  ...
  satisfied: true
  evidence: "xtask/src/spec_trace.rs:1818 — `pub(crate) fn clause_ids(root: &Path) -> Result<BTreeSet<String>>`, beside `all_rules` (:1746), reading the pinned `SPEC` through the existing `read` helper (:2307-2309); the decision half is the private `&str`-taking `collect_clause_ids` at :1831. No line of xtask/src/main.rs or xtask/src/lib.rs changed. Test xtask/src/spec_trace.rs:2437 ::tests::clause_ids_reads_the_pinned_specification — RED `the real document declares clauses; an empty set is the checker being broken`, GREEN with `ES-1` and `VT-1` present. `cargo test --locked -p xtask --bin xtask spec_trace::tests` → 8 passed."
...
- id: AC-002
  ...
  satisfied: true
  evidence: "xtask/src/spec_trace.rs:1831-1837 — `parse_clauses` → `clause.id` → `collect`, so the accepted prefixes are SECTIONS' (:122-153) by way of clause_id (:1391-1430); the diff introduces no clause-id regex and no prefix array. Tests xtask/src/spec_trace.rs:2460 ::tests::every_section_family_resolves (builds its slice by iterating SECTIONS, so a seventh family cannot leave it green by omission; RED `the VT- family must resolve; got {}`) and :2487 ::tests::both_declaration_forms_resolve_and_a_cross_reference_does_not (heading form, bold-run form, the `**CF-30 is [NON-NORMATIVE] …**` form, and `**ES-40**` rejected as a cross-reference; RED `ES-1 is a declaration; got {}`)."
...
- id: AC-003
  ...
  satisfied: true
  evidence: "xtask/src/spec_trace.rs:1831-1837 — no maturity filter, no family filter, and has_suite (:1729-1737) is not on the path. Test xtask/src/spec_trace.rs:2522 ::tests::a_deferred_clause_without_a_suite_still_resolves asserts both halves: a `[DEFERRED]` `SY-7` declaration resolves, and `has_suite(\"SY-7\")` is false — so a future edit routing the accessor through it fails this test rather than silently dangling every PS- and SY- citation. RED `a deferred clause is declared, so its id resolves; got {}`."
...
- id: AC-004
  ...
  satisfied: true
  evidence: "xtask/src/spec_trace.rs:1819 — `collect_clause_ids(&read(root, SPEC)?)`, so read's `reading {rel}` context propagates unchanged and the function never returns `Ok` with an empty set; :1838-1844 — the vacuity `bail!` in wire_rules' shape (:1784-1791), artifact name first, one composed sentence, no elision. Tests xtask/src/spec_trace.rs:2546 ::tests::an_unreadable_specification_names_the_file_it_could_not_read (RED `an unreadable document must never degrade to an empty set: {}`; GREEN with `reading spec/SPECIFICATION.md` in the `{:#}` chain) and :2561 ::tests::a_specification_that_declares_nothing_blames_the_checker, which asserts `message.starts_with(SPEC)`, the `the checker being broken rather than the document` wording, and that the message contains no `more`."
...
- id: AC-005
  ...
  satisfied: true
  evidence: "xtask/src/spec_trace.rs:1770-1802 — the doc block: the design's signed-off summary and sibling sentence (:1770-1773), `# What this does not verify` as the first heading (:1775) carrying the three limits, the alternative that lost named once (:1791-1796, RS-70-5), then `# Errors` phrased as two conditions rather than a type (:1797-1802). Tests xtask/src/spec_trace.rs:2587 ::tests::a_doubly_declared_id_collapses_to_one (RS-81-1's order — the headline limit proven before it is stated; RED `left: 0 right: 1`) and :2605 ::tests::the_accessor_documents_its_limits_before_its_errors, which reads the file through `read(&workspace_root()?, …)`, isolates the clause_ids doc block and asserts `does not verify` precedes `# Errors` and that no `badge`, `shield` or `✓` token appears (RED `the doc block states no limits at all`). Rendering proven green by cargo xtask ci --fast's documentation steps."
...
- id: AC-006
  ...
  satisfied: true
  evidence: "xtask/src/spec_trace.rs:1803-1817 — `#[cfg_attr(not(test), expect(dead_code, reason = …))]` whose reason names narrative-citation-resolution and frozen-documentation-must-pin; `grep -n \"allow(dead_code)\" xtask/src/spec_trace.rs` returns nothing. `cargo clippy --locked -p xtask --all-targets --all-features` is clean in both configurations and `cargo xtask ci --fast` printed `all required checks passed (--fast: 4 optional step(s) not run)` at this story's checkpoint. The `not(test)` scope is a recorded deviation (implementation report, Notes): the test module is itself a caller, so under cfg(test) the expectation is already unfulfilled and the gate's clippy step runs over every target. Self-erasure fires in the consumer's PR by design, as this spec's own clarification states."
