...
item: HS-S0145
stage: discover
created: 2026-08-17T06:12:16.170Z
updated: 2026-08-17T06:12:16.170Z
template_sig: 86ce4036
rendered_sig: 6c604603
---

# Discover — What the check does not verify is stated first, and executed where it can be
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `checked-documentation-surface/_storymap.md` | Put a "What this does not verify" section first in the new modules' own docs carrying all six enumerated limits, re-run the `RUSTDOCFLAGS` probe against the new step and record what it actually does, and walk a `text`-tagged broken fixture through the gate to prove that limit is real rather than asserted. |
| Project acceptance criteria this slice traces to | `checked-documentation-surface/_storymap.md` | AC-010 — the spec's AC-### must map onto these, not restate them |
...
| Dependency edges | `checked-documentation-surface/_storymap.md` | `fence-discipline-and-allowance-list`, `narrative-citation-resolution`, `frozen-documentation-must-pin`, `observed-failure-falsification` |
...
Put a "What this does not verify" section first in the new modules' own docs carrying all six enumerated limits, re-run the `RUSTDOCFLAGS` probe against the new step and record what it actually does, and walk a `text`-tagged broken fixture through the gate to prove that limit is real rather than asserted.
...
A limitations section that lists blind spots in prose with no accompanying demonstration, so a later reader cannot tell which are still real and which were closed.
