...
item: HS-S0159
stage: discover
created: 2026-08-17T06:31:33.708Z
updated: 2026-08-17T06:31:33.708Z
template_sig: 86ce4036
rendered_sig: e3bbffb7
---

# Discover — Somewhere for the second question to go
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `reach-and-adapter-path/_storymap.md` | Name the two second questions from DR-4's stall points and make each one hop from the page that answered the first, targeting the answering passage — using only intra-doc links, #[doc(alias)], rustdoc search and the destination's own TOC, and adding no navigation component. |
| Project acceptance criteria this slice traces to | `reach-and-adapter-path/_storymap.md` | AC-003, AC-005, AC-012 — the spec's AC-### must map onto these, not restate them |
...
Name the two second questions from DR-4's stall points and make each one hop from the page that answered the first, targeting the answering passage — using only intra-doc links, #[doc(alias)], rustdoc search and the destination's own TOC, and adding no navigation component.
...
behaviour, the AC-### enumeration mapping onto AC-003, AC-005, AC-012, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

A bespoke navigation widget where the gap is a missing link — the anti-pattern the dossier names explicitly, since rustdoc and the sidebar TOC already do the widget's job.
