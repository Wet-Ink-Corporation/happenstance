...
| `.bklg/docs-that-teach/application-author-path/invariant-to-appendcondition-bridge/_conditions.md` | This story's own named blocking conditions. **BC-001** is §6's heading: `_design.md:487-488` and AC-004 above both quote `## What a type-only guard misses`, which names the *opposite* failure to the one DT-6 resolved to and which AC-004's own body then describes correctly. BC-001 names `What a narrow guard misses` (`#what-a-narrow-guard-misses`) as authoritative pending the sign-off owner's call, records that this is a semantic rename rather than a budget retitle, and is where the call itself is recorded. | **First**, before §6's heading is typed — one string, one decision, without opening the upstream record. | AC-004 |
| `.bklg/docs-that-teach/application-author-path/tension-resolutions/_resolutions.md` | The upstream certification: what the design mandates and leaves blank, decided once. `§ DT-1` is the prior-model resolution whose single reader-facing home is this page's §3. `§ DT-5+DT-6` carries the wrong-side fence as a **compiled, executed** transcript — 23 rendered lines, 68 columns, nothing hidden, `assert!(accepted.is_ok())` passing, and exactly one expression (the guard's `QueryItem` tag) from the correct fence — plus the zero-allowance commitment AC-002 rests on. `§ Anchor table` fixes §2…§7's final heading text and the fragment id each emits, read off a render rather than predicted. `§ Consumption map` is this story's row. | Before writing any `##` on this page, and again before the §6 fence: copy the certified program's shape rather than re-deriving it from `_design.md:373-384`, which cannot compile and cannot be hidden into compiling. | AC-003, AC-004, AC-005, AC-006 |
