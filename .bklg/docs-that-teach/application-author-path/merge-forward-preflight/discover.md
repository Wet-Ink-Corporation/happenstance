...
item: HS-S0183
stage: discover
created: 2026-08-17T13:03:23.264Z
updated: 2026-08-17T13:03:23.264Z
template_sig: 86ce4036
rendered_sig: 2cc17891
---

# Discover — Merge forward from the sibling branch and record it
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `application-author-path/_storymap.md` | Merge `initiative/from-contract-to-published-library` forward and record the merge, so every page in this project is authored against the merged `crates/happenstance/src/lib.rs` rather than this worktree's 75-line copy. |
| Project acceptance criteria this slice traces to | `application-author-path/_storymap.md` | AC-014 — the spec's AC-### must map onto these, not restate them |
...
| Dependency edges | `application-author-path/_storymap.md` | none — this slice opens its project |
...
Merge `initiative/from-contract-to-published-library` forward and record the merge, so every page in this project is authored against the merged `crates/happenstance/src/lib.rs` rather than this worktree's 75-line copy.
...
behaviour, the AC-### enumeration mapping onto AC-014, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

Authoring against this worktree's 76-line `crates/happenstance/src/lib.rs` while the sibling branch carries the 237-line version with the defect intact. The work is redone at merge rather than merged.
