...
| `.bklg/docs-that-teach/application-author-path/tension-resolutions/_resolutions.md` | The slice-mate's certification — what `_design.md` mandates and leaves blank, decided once and measured rather than argued. `§ DT-4` certifies DT-4's three structural mitigations individually and **measures** step 3's program off a render: 24 rendered lines, 68 columns, 0 hidden, exactly at both ceilings, so it is the binding shape rather than a starting sketch and one added line breaks a budget. It also re-measures the payoff line at 93 characters of `Debug` and restates why the **output block**, not the string, must carry `ConditionViolated`. `§ Anchor table` supplies the retitle AC-002 needs — `Watch a boundary refuse` (23) → **`A boundary refuses`** (18, `#a-boundary-refuses`) — the three step headings with their emitted ids, and the finding that the crate root's `## Status:` section no longer exists. | Before the crate-root heading and before the first fence; again before measuring anything, so numbers are compared against that record rather than re-derived. | AC-001, AC-002, AC-003, AC-007 |
