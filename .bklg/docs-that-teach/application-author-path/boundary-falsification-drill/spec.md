...
| `.bklg/docs-that-teach/application-author-path/tension-resolutions/_resolutions.md` | `§ Anchor table` names this drill's own heading — **`Try it wrong, then put it back`**, emitting `#try-it-wrong-then-put-it-back` — which `_design.md:468-471` requires as a `###` and does not supply; picking a different one here would make the set-wide anchor review's table wrong. `§ DT-4` measures step 3's program at 24 rendered lines / 68 columns with **zero** headroom, which is the ceiling the drill's edit and its revert must not push it over. | Before writing the `###`, and before proposing any edit that adds a line to step 3's program. | AC-001, AC-006 |
