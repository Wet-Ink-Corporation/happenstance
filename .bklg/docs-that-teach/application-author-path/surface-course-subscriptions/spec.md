...
| `.bklg/docs-that-teach/application-author-path/tension-resolutions/_resolutions.md` | The two findings routed **here** rather than absorbed, plus the vocabulary this page must not re-argue. `§ Conditions and dispositions` row F3: the worked example's module doc says "aggregate" three times (`examples/course-subscriptions/src/main.rs:3,11,12`), inside material AC-001 requires surfaced **verbatim**, so the word DT-1 spends its single budget on lands on this page too — anti-pattern 6 does not govern this surface, the interaction is real, and both escapes are closed (paraphrasing by AC-001, editing the example by the UX brief), so how the seam is framed is this story's decision. Row D2: the seam sentence's premise — that the example imports `happenstance_core` — was deleted by the merge (`main.rs:34` imports `happenstance`). `§ DT-1` carries the one citation string to use if a prior model is named at all. | Before writing the seam sentence, and before deciding how the surfaced module doc is framed. | AC-001, AC-005 |
