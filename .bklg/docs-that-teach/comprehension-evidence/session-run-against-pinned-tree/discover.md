...
item: HS-S0165
stage: discover
created: 2026-08-17T06:40:16.717Z
updated: 2026-08-17T06:40:16.717Z
template_sig: 86ce4036
rendered_sig: 5ccd32e2
---

# Discover — Run the session against the named assembled tree
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `comprehension-evidence/_storymap.md` | Run the session against the post-merge assembled tree and record which tree that was, capturing concurrently what the reader did — searches typed, links followed, files opened, commands run — with severity marked inline, facilitator interventions timestamped as entries, keyboard reach recorded, and abandonment accepted as a valid end state. |
| Project acceptance criteria this slice traces to | `comprehension-evidence/_storymap.md` | AC-004, AC-005 — the spec's AC-### must map onto these, not restate them |
| Milestone / slice | `comprehension-evidence/_storymap.md` | `observed-session` — its slice-mates are delivered with it |
| Archetype | `comprehension-evidence/_storymap.md` | `capability` — a user-observable slice through every layer |
| Dependency edges | `comprehension-evidence/_storymap.md` | `friction-log-skeleton`, `non-insider-recruitment` |
...
Run the session against the post-merge assembled tree and record which tree that was, capturing concurrently what the reader did — searches typed, links followed, files opened, commands run — with severity marked inline, facilitator interventions timestamped as entries, keyboard reach recorded, and abandonment accepted as a valid end state.
...
behaviour, the AC-### enumeration mapping onto AC-004, AC-005, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

A session run against a half-assembled surface, which measures the assembly rather than the teaching.
