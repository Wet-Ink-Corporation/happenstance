...
item: HS-S0162
stage: discover
created: 2026-08-17T06:35:45.318Z
updated: 2026-08-17T06:35:45.318Z
template_sig: 86ce4036
rendered_sig: 7de4618c
---

# Discover — Resolve DT-9 and fix the protocol before recruitment
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `comprehension-evidence/_storymap.md` | Resolve DT-9 in _design.md — which persona the session walks, in terms of what that reader is trying to accomplish, with the two rejected personas and why each lost — and fix the scenario, narration mode, severity scale and logger-disqualifying criteria in the same commit, before any candidate is approached. |
| Project acceptance criteria this slice traces to | `comprehension-evidence/_storymap.md` | AC-001, AC-002 — the spec's AC-### must map onto these, not restate them |
| Milestone / slice | `comprehension-evidence/_storymap.md` | `session-protocol` — its slice-mates are delivered with it |
| Archetype | `comprehension-evidence/_storymap.md` | `capability` — a user-observable slice through every layer |
| Dependency edges | `comprehension-evidence/_storymap.md` | none — this slice opens its project |
...
Resolve DT-9 in _design.md — which persona the session walks, in terms of what that reader is trying to accomplish, with the two rejected personas and why each lost — and fix the scenario, narration mode, severity scale and logger-disqualifying criteria in the same commit, before any candidate is approached.
...
behaviour, the AC-### enumeration mapping onto AC-001, AC-002, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

A protocol settled after the session, fitted to what the session happened to produce.
