...
item: HS-S0163
stage: discover
created: 2026-08-17T06:37:07.295Z
updated: 2026-08-17T06:37:07.295Z
template_sig: 86ce4036
rendered_sig: eb278a7a
---

# Discover — The auditable-shape friction-log scaffold
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `comprehension-evidence/_storymap.md` | Land the auditable-shape log scaffold in-tree — scenario; logger identity, context and date; an append-only chronological section with stable occurrence-ordered stumble ids; a text-token severity mark on the named scale; a disposition slot at each stumble with a revision block; and the scope-sentence and hand-off sections — so the session fills a shape it did not invent while running. |
| Project acceptance criteria this slice traces to | `comprehension-evidence/_storymap.md` | AC-004 — the spec's AC-### must map onto these, not restate them |
| Milestone / slice | `comprehension-evidence/_storymap.md` | `session-protocol` — its slice-mates are delivered with it |
| Archetype | `comprehension-evidence/_storymap.md` | `foundation` — real in-tree substrate a capability slice consumes; never a double or a fixme |
| Dependency edges | `comprehension-evidence/_storymap.md` | `dt9-and-fixed-protocol` |
...
Land the auditable-shape log scaffold in-tree — scenario; logger identity, context and date; an append-only chronological section with stable occurrence-ordered stumble ids; a text-token severity mark on the named scale; a disposition slot at each stumble with a revision block; and the scope-sentence and hand-off sections — so the session fills a shape it did not invent while running.
...
A free-form diary with a date on it. The published method's shape — stated scenario, chronological record, reactions as they occur, severity inline — is what makes it auditable rather than anecdotal.
