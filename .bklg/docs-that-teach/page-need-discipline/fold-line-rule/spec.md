...
> trail and is **retracted here**. `dec82c7` committed 253 lines to that file,
> and every hunk of them is inside `mod tests` — tests pinning band `20` against the atom grammar, the five never-fold classes and the empty permitted-mechanism table. No production path,
...
the generator's format; `git diff main -- standards/rust/README.md` and `git diff main -- .kb` are
empty (`git diff main -- xtask` is **not** — see the amendment above); and
