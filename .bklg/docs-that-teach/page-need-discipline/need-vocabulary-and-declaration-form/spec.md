...
| repo-state | `rg -c 'allow\(dead_code' xtask/src/lint_pages.rs` → `1`; `rg -n 'page-need-checker-mounted-in-the-gate' xtask/src` → the reason string | AC-008 — one scoped allow, self-identifying, findable by the story that must delete it |
