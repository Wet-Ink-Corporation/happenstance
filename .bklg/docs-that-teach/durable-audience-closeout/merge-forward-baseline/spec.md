...
| --- | --- | --- | --- |
| `.bklg/docs-that-teach/application-author-path/merge-forward-preflight/_baseline.md` | The *other* merge-forward story's record — HS-S0183's, one project earlier — and specifically its `§ Carried forward to HS-P0025`, two process observations this story is the next to meet: **P1**, that story's PR boundary named its conflict-resolution paths *after* resolving them rather than before, against its own EC-001 (name them first here, and record the delta against `git diff-tree --cc --name-only <merge>` either way); and **P2**, that `redkiln verify --grain story` cannot express "arrived by parentage", so a merge commit's second parent fails the boundary check on files nobody authored. Its `§ Merge`, `§ Anchors` and `§ Dispositions` are also the shape this record's own sections were modelled on. | Before running the merge — P1 is only meetable beforehand — and again when the boundary check fails on parented files. | AC-004 |
