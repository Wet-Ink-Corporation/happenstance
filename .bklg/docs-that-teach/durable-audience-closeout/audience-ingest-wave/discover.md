...
item: HS-S0178
stage: discover
created: 2026-08-17T06:56:08.092Z
updated: 2026-08-17T06:56:08.092Z
template_sig: 86ce4036
rendered_sig: df16a7a6
---

# Discover — Run the single closeout ingest wave into .kb/product/
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `durable-audience-closeout/_storymap.md` | Run the single closeout `/redkiln:kb-ingest` wave against an explicit file list that excludes `.kb/_intake/README.md`, landing persona atoms as `kind: concept` / journey atoms as `kind: playbook` — both `authority_tier: product` — plus HS-P0021's carried playbook payload, with resolving `source_paths`, a cleared intake directory and `redkiln validate --kb` green. |
| Project acceptance criteria this slice traces to | `durable-audience-closeout/_storymap.md` | AC-004, AC-005, AC-008, AC-009, AC-011 — the spec's AC-### must map onto these, not restate them |
| Milestone / slice | `durable-audience-closeout/_storymap.md` | `product-layer-promotion` — its slice-mates are delivered with it |
| Archetype | `durable-audience-closeout/_storymap.md` | `capability` — a user-observable slice through every layer |
| Dependency edges | `durable-audience-closeout/_storymap.md` | `staged-audience-payload` |
...
Run the single closeout `/redkiln:kb-ingest` wave against an explicit file list that excludes `.kb/_intake/README.md`, landing persona atoms as `kind: concept` / journey atoms as `kind: playbook` — both `authority_tier: product` — plus HS-P0021's carried playbook payload, with resolving `source_paths`, a cleared intake directory and `redkiln validate --kb` green.
...
behaviour, the AC-### enumeration mapping onto AC-004, AC-005, AC-008, AC-009, AC-011, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

Atoms hand-written into `.kb/product/` outside the ingest path, reverted once already at `0269720`.
