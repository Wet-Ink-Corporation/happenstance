...
item: HS-S0179
stage: discover
created: 2026-08-17T06:57:12.548Z
updated: 2026-08-17T06:57:12.548Z
template_sig: 86ce4036
rendered_sig: 14deae8b
---

# Discover — Mount every promoted atom at all four mount points
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `durable-audience-closeout/_storymap.md` | Mount every landed atom at all four points — an appended `##` section in `.kb/maps/domain-map.md` with no deletion line anywhere in the diff, reciprocal `related` / `depends_on` edges, `links.kb` on HS-P0025 via `redkiln record-links --atom`, and a `## Knowledge Harvest` row — and record the two-hop reachability walk from `.kb/README.md`. |
| Project acceptance criteria this slice traces to | `durable-audience-closeout/_storymap.md` | AC-010, AC-011 — the spec's AC-### must map onto these, not restate them |
...
| Dependency edges | `durable-audience-closeout/_storymap.md` | `audience-ingest-wave` |
...
Mount every landed atom at all four points — an appended `##` section in `.kb/maps/domain-map.md` with no deletion line anywhere in the diff, reciprocal `related` / `depends_on` edges, `links.kb` on HS-P0025 via `redkiln record-links --atom`, and a `## Knowledge Harvest` row — and record the two-hop reachability walk from `.kb/README.md`.
...
behaviour, the AC-### enumeration mapping onto AC-010, AC-011, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

Promoted atoms nothing links to, so the next initiative re-derives an audience it cannot find.
