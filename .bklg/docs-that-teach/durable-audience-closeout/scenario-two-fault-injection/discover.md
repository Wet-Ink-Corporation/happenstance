...
item: HS-S0181
stage: discover
created: 2026-08-17T06:59:23.691Z
updated: 2026-08-17T06:59:23.691Z
template_sig: 86ce4036
rendered_sig: df124150
---

# Discover — Re-observe scenario 2 in both halves: fail by name, then recover
...
| ------ | ------ | ----------- |
| The slice, as the story map states it | `durable-audience-closeout/_storymap.md` | Re-observe DoD scenario 2 in both halves: break the pinned page deliberately, capture the gate's failing-by-name output verbatim as the failing half's evidence, revert, and re-run green as the recovery half — with the broken edit never a committed state of the tree. |
| Project acceptance criteria this slice traces to | `durable-audience-closeout/_storymap.md` | AC-015, AC-014 — the spec's AC-### must map onto these, not restate them |
...
| Dependency edges | `durable-audience-closeout/_storymap.md` | `dod-scenario-ledger` |
...
Re-observe DoD scenario 2 in both halves: break the pinned page deliberately, capture the gate's failing-by-name output verbatim as the failing half's evidence, revert, and re-run green as the recovery half — with the broken edit never a committed state of the tree.
...
behaviour, the AC-### enumeration mapping onto AC-015, AC-014, and the integration contract naming
the real mount point — none of which is decided here.

## The wrong implementation

Scenario 2 recorded as passing because the gate is green, which is the one scenario whose whole point is the failing half.
