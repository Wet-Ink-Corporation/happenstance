...
| **AC-005** — three written verdicts, no level moved | **Met** | ES-35's falsifier restated to the **fault** far end; CF-17's rule shape **confirmed**; CF-14's deferral **confirmed and narrowed** to HS-P0013 and HS-P0014. `spec-trace`'s census identical before and after; the three status rows unmoved; the escalation note is `_adr-queue-escalation.md` beside this file. **Corrected after review**: §6.5's summary paragraph, §1.3's qualification, §1.7's caveat and §1.6's `EventStore` row all still said the adapter column was empty on every axis, which this story's own row edit had falsified — all four now state the same count, and the handle-multiplicity row is routed to `spec-and-code-reconciliation` rather than moved here |
