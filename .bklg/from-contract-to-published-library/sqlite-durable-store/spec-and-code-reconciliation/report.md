...
| --- | --- | --- | --- |
| **AC-001** — a verdict for every clause in the computed range | **satisfied** | 8 clauses in set A + 24 further passages whose text names this crate + 11 more that the mechanical re-run of the widened rule turned up and nothing was done to; 17 unchanged, 26 repaired, none passed silently. **Eight** sentences whose MUST was untouched and whose prose described a superseded implementation are recorded as **defects**, not passes — and the eighth is one this pass wrote itself | `_reconciliation.md`, *Clause-by-clause verdicts* |
| **AC-002** — every pointer lands on its stated subject | **satisfied** | Four schema-sketch citations re-anchored `:47-53` → `:62-67`; the projection census corrected in two places; ES-41's spent falsifier restated; D7's read path read against ADR-0022's settled runtime seam; five further drifted pointers found that no spec named in advance. **None deleted** — `checked` rose 389 → 401 | `spec/SPECIFICATION.md` |
| **AC-003** — every edit is a repair | **satisfied** | 202 insertions / 85 deletions; **zero** deleted lines carry the normative token MUST and two carry a lower-case "must", both read (one table row whose PS-2 column is reproduced verbatim, one prose sentence). Two met obligations named **as** discharges with code and test cited | `spec/SPECIFICATION.md` |
...
| **AC-005** — neither citation figure falls | **satisfied**, after a real red | 389 → 401 checked, 76 → 80 anchored. `anchored` fell to 75 mid-pass (EC-004) and was diagnosed and re-anchored rather than absorbed; `ANCHOR_SLACK` untouched | `spec/SPECIFICATION.md`, `_reconciliation.md` |
...
None was reported, and the tool is not at fault. Of 401 citations checked, **80**
...
line or the one above. The other 321 are counted and unexamined. A reader who
