...
- id: AC-005
  criterion: >-
    ...
    per chunk.
  satisfied: true
  evidence: "CORRECTED, and the earlier evidence for this row was wrong. It recorded AC-005 as NOT satisfied, on the conclusion that ES-11 cannot be met by an asynchronous driver at all -- the first poll can only start the round trip that takes the snapshot. That conclusion was drawn from five consistent failures and it was WRONG. Both `read_result_is_stable_under_concurrent_append` and `query_items_share_one_snapshot` now pass, 5 runs out of 5, and the whole gated suite is 101/101 green. What fixes it: every operation is handed to the runtime rather than polled inline -- `open_cursor` is spawned at the first poll and `append` is spawned through `on_runtime`, so the read's task is enqueued first and its snapshot is taken before the append runs. The frontier predicate is composed into the cursor's `DECLARE` inside one `BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY` and evaluated once, never per `FETCH`. The bridge was built for an unrelated reason (sqlx panics without a runtime on the concurrency family's raw contender threads), which is why the wrong diagnosis survived: the thing that would have disproved it had not been written yet. The incorrect `.kb/_intake/` note asserting the falsification has been deleted rather than ingested."
...
- id: AC-008
  ...
  satisfied: true
  evidence: "The two invocations plus the model family are mounted in `crates/happenstance-postgres/tests/postgres_conformance.rs` over their FULL expansion, each gated whole by its own `#[ignore]`-adding emitter passed as the macro's `emit =` parameter -- no rule is `#[cfg]`-ed out of any expansion. `-- --ignored --list` reports `101 tests, 0 benchmarks` -- 89 event-store rules, 5 concurrency, 1 model, and this crate's own 6. Live run, `-- --ignored --test-threads=1`: `101 passed; 0 failed`. The two non-ignored no-server tests make up the rest. NOTE the correction recorded under AC-005: an earlier run of this suite reported 93/95 with two ES-11 failures, and both the number and the conclusion drawn from it were superseded once every operation was routed through the runtime."
