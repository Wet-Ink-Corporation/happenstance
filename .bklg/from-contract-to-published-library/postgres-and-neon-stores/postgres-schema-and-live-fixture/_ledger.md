...
- id: AC-007
  ...
  satisfied: true
  evidence: "CORRECTED. The first evidence written here cited `--ignored --list` reporting `95 tests, 0 benchmarks` and called that the full expansion. It was not: 95 is the event-store family's 89 rules plus this story's own 6 gated tests, and `event_store_concurrency_conformance!` and `event_store_model_conformance!` were never mounted at all. The count was checked without checking what it was made of -- the same failure the gating in this story is written against, one level up. Both families are mounted now and `--ignored --list` reports `101 tests, 0 benchmarks`: 89 `dcb_conformance`, 5 `dcb_concurrency_conformance`, 1 `dcb_model_conformance`, 6 story-local. The testkit dev-dependency carries `features = [\"proptest\"]`, without which `event_store_model_conformance!` does not exist and the mount drops a family SILENTLY rather than failing to compile. Gating stays whole-invocation: three local emitters (`emit_ignored_tokio`, `emit_ignored_concurrency_tokio`, `emit_ignored_model_tokio`) passed as each macro's `emit =` parameter, each adding `#[ignore]` to every generated test and hiding no rule from any expansion -- DR-5's distinction, kept."
  mount_point: "crates/happenstance-postgres/tests/postgres_conformance.rs"
