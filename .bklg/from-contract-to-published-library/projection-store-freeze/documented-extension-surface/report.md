...
| **AC-004** | **Met** | `.../outside_projection_discrimination.rs` — `commit_is_atomic_with_the_read_model` fails `CheckpointOnlyStore` by name; the trap test and the control beside it. Reached via the published `projection::rules` path. | `examples/outside-projection-adapter/src/lib.rs:302-402` |
...
| **AC-008** | **Met** | `cargo xtask ci`: `all checks passed`. Two gaps fixed and cited (G6, G9); three reported, not absorbed (G1, G2, G5). `Cargo.lock` committed. | `contract.rs:891-905`, `standards/rust/`, `CLAUDE.md`, `CHANGELOG.md` |
...
- **One declared boundary widening**: G9's three `standards/rust/` files, forced by the mount point
  the spec chose and unavoidable by any placement.
