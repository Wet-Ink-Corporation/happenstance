...
**AC-010's instrument was amended on 2026-08-14; its premise was not.** It read
`git diff --stat main...HEAD -- spec/` **empty**, which no story that adds or
removes a rule name can satisfy: `cargo xtask spec-trace --write` regenerates
§7.1–§7.2's `BEGIN/END GENERATED` region, a `†` moves whenever a rule starts or
stops existing, and a *separate* gate step fails on a stale region — so the
criterion demanded that a story leave stale exactly what the gate demands it
regenerate. This slice tripped it on 18 lines, every one a generated `†`.

What AC-010 always meant is written in its own next clause — *no `[FROZEN]`
clause text changed and no maturity marker moved to make anything pass* — and
that is what it now says, scoped to hunks **outside** the generated region. The
change is a **tightening, not a loosening**: the old form was unmeetable and was
therefore going to be ratified away per story, five times so far across three
slices, each ratification eroding what the criterion forbids. The new form is
verifiably true here and still rejects the thing it exists to reject — a
hand-edited clause, a moved marker, or a citation rewritten to make a rule look
covered. Machine-regenerated content inside the region is independently policed
by spec-trace's own stale check, so nothing is now unwatched that was watched
before.
