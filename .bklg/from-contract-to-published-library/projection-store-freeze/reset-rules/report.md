...
**Ten of ten ACs satisfied. Nothing deferred, nothing stubbed, no clause text
edited.** Five rules, six wrong stores, three grown declarations, and one new
provided method on the projection fixture trait that is a mechanism rather than a
capability.
...
| **AC-004** | **satisfied** | `projection::a_declined_reset_refusal_is_reported_with_the_fixtures_reason` asserts the `RuleOutcome` **value** — `Skipped { capability: "RESET_REFUSAL", reason }` — with the reason read from `MemoryProjectionFixture`'s own `const`, never from a repeated literal and never from stdout. `mutation_coverage::projection_capability_skips_are_reported` pins the reference fixture's whole skip set to exactly two rules and checks each one's capability and reason. No projection-local skip type was invented. | `crates/happenstance-testkit/src/projection.rs:1753`; `…/tests/mutation_coverage.rs:3546` |
| **AC-005** | **satisfied** | `fresh_projection_has_no_checkpoint` asserts the `Checkpoint::NeverRun` **variant** and compares no position anywhere. Red against `PresumedLiveCheckpointStore`. **Precondition checked before the rule was written**: ADR-0018 (`.kb/decisions/0018-returning-a-projection-to-never-run.md`) is `status: accepted` and `redkiln validate --kb` is green on this tree, so EC-001 did not fire. | `…/projection.rs:1567` |
| **AC-006** | **satisfied** | `reset_is_not_commit_at_first` — both halves. The **state** half compares `core::mem::discriminant` of the two checkpoints; the **consequence** half derives a resume point from each through `resumes_over` (strictly after a recorded position, inclusive from the first position when `NeverRun`) and asserts the event at the first position is applied after the reset and *excluded* after the substitute. Red against `CommitAtFirstResetStore`. No runner was built. | `…/projection.rs:1568` |
| **AC-007** | **satisfied** | `TruncatingResetStore`'s row: `fails: &["reset_is_scoped_to_one_projection"]`, non-empty provenance naming the truncating `SqliteProjectionStore::reset()` and the `van_stock` / `fgas_ledger` consequence, and an `expect` pin. Held by all four projection exactness meta-tests. | `crates/happenstance-testkit/tests/projection_mutation_coverage.rs:581` |
| **AC-008** | **satisfied** | Every one of the five rules has at least one registered wrong store failing it, and each of the six new stores overrides **one** `Defect` step: `reset_writes` (three), `refusal` (two), `missing_checkpoint` (one). No pass rate is computed or quoted, here or in the binary. | `…/tests/projection_mutation_coverage/mutants.rs:299-460`; rows at `…/projection_mutation_coverage.rs:544-700` |
| **AC-009** | **satisfied** | All five names live in the one enumeration, so the tokio, blocking and `wasm32` harnesses each gained five tests with no per-harness list. `no_orphan_projection_rules` makes an unmounted rule a test failure; `cargo check -p happenstance-testkit --tests --target wasm32-unknown-unknown` type-checks the wasm harness in the same gate run. | `…/projection.rs:1563-1568`; `…/tests/projection_conformance_wasm.rs` |
| **AC-010** | **satisfied** | `cargo xtask spec-trace` green (check 6 resolves all five names; `RULE_FILES` already named `projection.rs`, so EC-002 did not fire); the CF-29 changelog lint green over five entries each naming a defect; the CF-6 position lint green. `git diff -- spec/` is **the generated §7.1–§7.2 region and nothing else** — five `†` markers removed by `spec-trace --write`, no clause text, no maturity marker, no rule citation. | `CHANGELOG.md`; `spec/SPECIFICATION.md` generated region |
