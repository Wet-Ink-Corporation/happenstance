...
> **AMENDED BY THE SLICE FIX PASS, 2026-08-14.** The status below was written at
> the implement commit and is superseded on one point: **AC-005 is blocked, not
> satisfied.** EC-001 fired — the PS-19 repair atom its precondition names does
> not exist — and `fresh_projection_has_no_checkpoint` and
> `PresumedLiveCheckpointStore` are **held** rather than shipped. Every row below
> that counts five rules or six stores counts four and five after that pass. The
> reasoning is in `_slices.md`, *reset-and-rebuild-rules — fix pass 2026-08-14*,
> and the corrected rows are in `_ledger.md` and `report.md`. Nothing else in
> this document changed: it is the record of the run, not of the tree.
>
...
> the new rules being weakened to preserve them.
>
> **What the projection suite is after this commit.** Fourteen rules, not nine —
> thirteen after the fix pass held one. `reset` is now a checked operation rather
> than a signature: one unit of work covering rows and checkpoint together,
> scoped to one `(store, ProjectionId)`, refusable with no side effect, and — the
> one this story exists for — **not** `commit(empty, id, FIRST)`. The substitute
> six deployment scenarios out of six reached for now fails a named test with a
> resume-point consequence attached. *"`NeverRun` for an id nobody has ever
> committed"* was in this list and is not delivered: no clause obliges it yet.
...
| AC-005 | — | **WITHDRAWN by the slice fix pass, 2026-08-14. EC-001 fired and this row's original claim was false.** The precondition AC-005 states is *the PS-19 repair atom from `projection-decision-atoms`*, accepted under `.kb/decisions/`; no such atom exists. This row cited ADR-0018's `status: accepted`, which is a different atom that explicitly repairs nothing here (`.kb/decisions/0018-…:113-116`). `fresh_projection_has_no_checkpoint` and `PresumedLiveCheckpointStore` are **held** until the widening decision lands at `unstable-projection-gate-and-clause-disposition`. See `_slices.md`, *reset-and-rebuild-rules — fix pass 2026-08-14*. |
...
| `crates/happenstance-testkit/tests/projection_mutation_coverage/mutants.rs` | **Five new hostile stores** after the fix pass, each an `impl Defect` overriding exactly one step: `TwoStatementResetStore`, `TruncatingResetStore`, `CommitAtFirstResetStore`, `RefusalAsSuccessStore`, `RefusalAfterTheFactStore`. Each was named defect-first, from §4.11's *Rejects* column and from `RUNBOOK.md:3904-3907`, before it was named at all. A sixth, `PresumedLiveCheckpointStore`, is **held** with AC-005's rule — it is conformant with PS-19 as the clause is written today, so declaring it a mutant asserts an obligation no clause states. |
| `crates/happenstance-testkit/tests/projection_mutation_coverage.rs` | Five `Declared` rows after the fix pass (`:554-686`; a sixth held), and **three existing rows grown** (`UncommittedTransactionStore`, `TypeStampedBatchStore`, `ValidatingCommitStore`) with a comment on each naming the repair that was refused. Five entries added to `for_each_projection_mutant!`. |
...
| `spec/SPECIFICATION.md` | **The generated region only.** `cargo xtask spec-trace --write`; every hunk a `†` removed from PS-16 – PS-20 as its rule started existing — and, after the fix pass, one restored on PS-19's held rule. No clause text, maturity marker or rule citation moved. |
