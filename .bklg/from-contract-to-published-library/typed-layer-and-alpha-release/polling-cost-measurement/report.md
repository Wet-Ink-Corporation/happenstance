...
| AC-003 | satisfied | `record_conditions.rs` (6) | 17 fields on every record, 15 on the once-per-pass manifest, none null or empty. The committed pass names revision `3999b5a` with `git_dirty: false` and `profile: release`. **Two conditions were re-taken after review** — see *Deviations*: `started_at` reported the commit instant under a field documented as the run's, and `ram_bytes` was a constant zero under a field documented as a query. Both are now captured and both are held by tests that were red against the earlier pass. |
...
| AC-009 | satisfied | `readme.rs` (4) + `output_format.rs` (4) | Sections in the declared order, ratio before the first timing figure, `… and 30 more` on both long tables. The overflow check is scoped to the prose following the table that overflowed — review found it asking the whole document, so one marker satisfied every future overflow — and the wrong document it used to accept is now a test beside it. §1's thirteen-row conditions table, which passed only through that loophole, is split into *the measurement* and *the machine*. Console output is plain lines, **truncated** at 80 columns rather than wrapped, with control characters flattened. |
