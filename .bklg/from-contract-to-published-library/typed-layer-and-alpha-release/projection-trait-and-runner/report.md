...
| AC-004 | satisfied | `::first_run_starts_from_the_beginning`, `::resume_advances_past_the_checkpoint`, `::tolerates_gapped_positions` | `NeverRun` means no `from` anchor at all, not position zero. Resume is `through.next()`, whose `None` arm is `ProjectionError::KeySpaceExhausted`, never an `unwrap`. The gap test runs against `GappyMemoryStore` at stride 7 and names no literal position. |
