...
| AC-005 | satisfied, **and made non-vacuous 2026-08-19** | `::every_declined_capability_names_this_runtime`. Red beat: `declined("not decided yet")` rejected by name. The guard's only subject was this fixture's one decline, and `measured-store-limits` then removed it — leaving a walk that skipped its own body three times and asserted nothing. It now walks a `DecliningFixture` defined in the same target, with a floor assertion on subjects examined | `tests/support/mod.rs`; `tests/fixture_contract.rs::DecliningFixture` |
| AC-006 | satisfied, **and made non-vacuous 2026-08-19** | `::on_the_object::a_supported_capability_has_its_method_overridden` (reaches the provided body's panic path for every supported capability), `::mid_batch_fault_is_restated_not_inherited`. Red beat: the inherited default printed identical on both sides. The second test degenerated to `None != Some(default)` once the capability became `SUPPORTED`; it now also reads the impl's source, because **inheritance is an omission** and no value can show it | `tests/support/mod.rs`; `tests/fixture_contract.rs` |
