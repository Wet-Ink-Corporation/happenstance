...
> **Amended 2026-09-02 by the crate-set re-plan.** This spec was written against
> a three-crate release and its acceptance criteria asserted that number. The
> deployment brief's decision has since been re-made — four crates, adding
> `happenstance-sqlite` — under its own escape clause, and the artefacts that
> clause requires to move together have moved:
> `publication-and-positioning/_decomposition.md`'s *The crate-set decision*,
> this project's `_intake-brief.md`, `xtask/src/package.rs`'s `PUBLISHABLE` and
> the crate's own manifest.
>
> The paragraphs and criteria below are **amended in place rather than
> regenerated**, because a spec carrying an amendment banner over criteria that
> still assert the superseded count is precisely the *"reads correctly while
> nobody ran the check"* failure this story names as its own wrong
> implementation. What is **not** amended here is anything downstream of the
> decision itself — the atom's shape, the frontmatter, the map row and the
> `redkiln` hygiene are unchanged.

**1. The set is four, and the names are fixed.** `0.2.0` ships `happenstance-core`,
`happenstance`, `happenstance-testkit` and `happenstance-sqlite` — the deployment brief's re-made
decision (`publication-and-positioning/_decomposition.md`, *The crate-set decision*).
`happenstance` is the crate a consumer installs and the one holding the bare name
(`.kb/decisions/0006-bare-name-to-the-typed-layer.md`); `happenstance-core` is what an adapter author
pins; `happenstance-testkit` is the bar an adapter clears; `happenstance-sqlite` is the first adapter
to have cleared it, and the reason an evaluator can store an event without writing a store first.

**1a. Do not read that number off `PUBLISHABLE`.** The constant is the publish-*ready* set and names
**five**, because `cloudflare-durable-object-store`'s `publish-ready-crate` story promoted
`happenstance-cloudflare` and explicitly stopped there, leaving the release decision here. The
release set is four; `happenstance-cloudflare` is **deferred past `0.2.0`** by a dated decision of
2026-09-02 recorded in the deployment brief, and is not settled by the constant either way. The original of this paragraph said the set
"matches `xtask/src/package.rs`:86", which conflated the two and is the sentence this one replaces.

**2. The alternative that lost is three crates, and it must be named with its cost.** The deployment
brief settled on `happenstance-core`, `happenstance` and `happenstance-testkit` on 2026-08-12 and
rejected four on three grounds. Those grounds are **answered, not deleted**, and the answers are
carried forward rather than re-derived:

- *"A fourth is a change to that constant, which this project must take deliberately."* **Upheld.**
  The ground was about how the set may move, not about `happenstance-sqlite`, and the re-plan is the
  deliberate change it asked for. The constant and the manifest moved together, which is the only
  order `reconcile` is ever green in.
- *"A fourth crate owes licences, a README, a status-table row, a disambiguation entry and a docs.rs
  configuration — uncosted scope."* **Discharged in substance.** The licences, README and description
  landed with `sqlite-durable-store`'s `crates-io-name-and-packaging-facts`; the docs.rs
  configuration landed with the re-plan. `cargo publish --dry-run -p happenstance-sqlite` verified
  against `happenstance-core` **resolved from the registry**, which is the half a workspace check
  cannot reach. The two narrative items are real, are now carried in `_intake-brief.md`'s scope, and
  belong to HS-S0092 and HS-S0094 rather than to this story.
- *"`RUNBOOK.md`'s phase-12 section is independently known stale."* **Withdrawn as a ground.** The
  staleness is real, is in the *clause count*, and was used to discount an unrelated sentence in the
  same section. `RUNBOOK.md`:4450-4451 is now the source being **followed**.
...
| **Renders surfaces** | **none.** `_design.md`'s `## Items` (`:49-79`) carries no item this story adds and `## Surfaces` (`:104-176`) names no surface it renders. The honest coupling is one level down, and the re-plan settled it: the `crates-io-*` and `docs-rs-*` surfaces exist as one pair per published crate, so the fourth crate added a fourth pair — `crates-io-happenstance-sqlite` and `docs-rs-happenstance-sqlite`, both now in the signed-off design as non-primary surfaces sharing the existing skeleton. This story still renders none of them |
...
| **B8 — the derived set is five because four crates decline to publish** | The evidence records *why* the derived set is what it is: `publish = false` at `crates/happenstance-{ladybug,postgres,neon,sync}/Cargo.toml`, plus `examples/*` and `xtask`. `happenstance-sqlite` and `happenstance-cloudflare` no longer carry it — the first by the crate-set re-plan, the second by `publish-ready-crate` — which is what makes "a `publish = false` gone missing" checkable in the *withdrawn* direction too, and why AC-004 now seeds both | `crates/happenstance-ladybug/Cargo.toml` and its three siblings; `xtask/src/package.rs`:24-42 |
...
| --- | --- | --- |
| AC-001 | **GIVEN** the repository owner has decided once that `0.2.0` ships exactly `happenstance-core`, `happenstance`, `happenstance-testkit` and `happenstance-sqlite`, **WHEN** they (or any later reader) open `.kb/decisions/`, **THEN** an accepted decision atom at a number ≥ 0030 states that set — **exactly four names, listed** — in the decision-atom shape of record (`.kb/decisions/0029-msrv-raised-to-1-97-1.md`:1-33): valid `KbFrontmatter` with `kind: decision`, `authority_tier: decision`, `status: accepted`, `supersedes: null`, `superseded_by: null`, a `related` edge to `kb-decision-0006` and `source_paths` naming the intake file, `xtask/src/package.rs` and `RUNBOOK.md`; **AND** the atom defines the vocabulary it uses — *intention list* (`PUBLISHABLE`), *derived set* (`cargo metadata`) and *release set* (what `0.2.0` uploads) — where it uses them, the third because the re-plan found the first two being read as the third; **AND** the atom states plainly that the release set is four while the publish-ready set is five, and that `happenstance-cloudflare` is **deferred past `0.2.0`** by a dated decision the atom states rather than leaves to be inferred from its absence | `redkiln validate --kb` exits 0 with the new atom present; a content check that the atom names all four crates, carries the six frontmatter fields above, defines all three terms inline, and contains the sentence distinguishing the release set from `PUBLISHABLE`, and a sentence deferring `happenstance-cloudflare` with its date; `git ls-files .kb/decisions/` shows exactly one added path and its number is ≥ 0030 and not in 0017–0028 |
| AC-002 | **GIVEN** the deployment brief settled the release at three crates on 2026-08-12 and rejected four on three named grounds, **WHEN** the owner or a future planner reads the new atom, **THEN** an *Alternatives rejected* section names **three crates** as the alternative that lost, quotes the three grounds from `publication-and-positioning/_decomposition.md`'s *The crate-set decision*, and records what happened to each — ground 1 upheld and satisfied by the re-plan, ground 2 discharged in substance with the two narrative items re-homed to HS-S0092/HS-S0094, ground 3 withdrawn with its clause-count staleness still owed elsewhere; **AND** the atom names `RUNBOOK.md`:4450-4451 as the source now **followed** rather than overridden, which is the reverse of what this criterion asked for before the re-plan; **AND** each disposition is readable *in the atom* with one hop to its evidence (IQ-1) | a content check that the atom contains the three-crate string, all three grounds, and a disposition word (`upheld`, `discharged`, `withdrawn`) against each; a link/citation check that every `file:line` in that section resolves in the tree at HEAD; a check that the atom does **not** contain the word `overridden` applied to `RUNBOOK.md` |
| AC-003 | **GIVEN** the atom claims the intention list and the manifests agree, **WHEN** the owner needs that claim to be checked rather than trusted, **THEN** `cargo xtask package-check` has been run at the decision commit and its **stdout is a committed artefact** under this story's folder, carrying the commit sha, the date, the literal command line, the exit status, `reconcile`'s success line naming the **five** publish-ready crates (`xtask/src/package.rs`:213-217) and the five per-crate lines naming `LICENSE-MIT`, `LICENSE-APACHE` and `README.md`; **AND** the capture is read as evidence for the *publish-ready* claim only — the atom must not cite it as evidence of the four-crate **release** set, which no gate step can observe; **AND** the atom cites the artefact rather than restating `PUBLISHABLE`'s contents (IQ-5) | the committed capture exists, exit status is 0, and its `reconcile` line names exactly the five publish-ready crates; `cargo xtask package-check` re-run at HEAD reproduces it; `cargo test -p xtask package::tests` (`xtask/src/package.rs`:408-457) is green |
| AC-004 | **GIVEN** the named wrong implementation is an atom that reads correctly while nobody ran the check (`discover.md`:38-40), **WHEN** the check is seeded with drift in **each** direction in the working tree only — *promoted*: `publish = false` deleted from `crates/happenstance-ladybug/Cargo.toml`, a crate with none of the three required files; *withdrawn*: `publish = false` **added** to `crates/happenstance-sqlite/Cargo.toml`, whose name the constant now carries — **THEN** `cargo xtask package-check` fails on each, naming the direction, because the two directions are two different bugs (`xtask/src/package.rs`:163-204); **AND** both seeded edits are reverted and appear in no commit; **AND** if the *unseeded* run had failed in either direction, the story stops and records it rather than editing `PUBLISHABLE` or a manifest to make the gate green (IQ-4). The pre-re-plan spelling of this criterion seeded the promoted direction by deleting `publish = false` from `happenstance-sqlite`; that edit is now the committed state and seeds nothing, which is why the seed moved to `happenstance-ladybug` and gained its opposite | both seeded-failure transcripts are committed alongside AC-003's passing one, each naming its direction; `git diff --quiet -- crates/` and `git diff --quiet -- xtask/` at HEAD prove both seeds were reverted |
...
| AC-006 | **GIVEN** the plan of record states a four-crate goal that the re-plan now **agrees** with, **WHEN** a reader arrives at `RUNBOOK.md`'s phase-12 **Goal** paragraph, **THEN** they meet a single added pointer line naming the atom as the source in force for the crate set — the sentence is **annotated, not deleted** (IQ-2), no heading is renamed and no anchor moves (IQ-3), and no other region of `RUNBOOK.md` — the provisional falsifier ledger at `:622-635` above all — is entered; **AND** the annotation says the goal is *confirmed* by the atom rather than *superseded* by it, which is the opposite of what this criterion asked before the re-plan; **AND** the three stories that block on this one (`registry-surface-diff`, `landing-copy-and-status-truth`, `publish-0-2-0`, `_storymap.md`:105) can quote the set from the atom without re-deriving it | `git diff RUNBOOK.md` shows added lines only, inside `:4448-4460`, with no heading line changed and no line removed; a heading-anchor check that every `#`-anchor `RUNBOOK.md` exposed before the edit still exists; the atom's decision sentence is quotable verbatim as the three consumers' input |
...
- **The disambiguation triad's `≤ 3 entries` budget** (`_design.md`:474; region R3 at `:359`; the
  14-rendered-line first screen at `:452-456`) exists *because* the published set is small and each member owes a first screen — the
