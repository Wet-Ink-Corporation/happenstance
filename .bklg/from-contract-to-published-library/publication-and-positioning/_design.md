...
| R5 | **Badges** | CI + licence on GitHub; a *buy me a coffee* badge on **both** surfaces | **Amended 2026-09-02 by the sign-off owner.** It read *"decoration only"* and *"below R4, not under the H1"*. The support badge is deliberately **under the H1**, on the six pages, because a funding link nobody sees funds nothing. The rest of R5 is unchanged: CI and licence stay decoration and stay below R4 |
...
| Badges | **persistent but non-load-bearing** on GitHub. On crates.io: CI and licence **revealed on scroll**, the support badge **persistent chrome** | They are images, and the page must still read correctly with images blocked — the support badge's alt text carries its own label, so nothing depends on the render. The second clause used to end *"on crates.io they cost first-screen budget the triad needs"*, and that cost is now **paid deliberately** rather than avoided: see the *Density budget* row below, which prices it |
...
| Support badge | 1 rendered line + 1 blank, **above the fold** | **Added 2026-09-02, and it inverts this table's own demotion order.** The order below still reads *badges, then the census, then compressing the identity line* — badges first to go. That is no longer true of this one: it is the region the sign-off owner put above the fold on purpose, so it is the region that does **not** yield. Whoever writes the final crates.io copy inherits 12 of the 14 rendered lines, not 14, and must re-read the demotion order against that rather than against this row's absence |
| Identity line | ≤ 2 rendered lines | Leaves 10 of 14 for callout + triad, once the support badge's 2 are spent |
