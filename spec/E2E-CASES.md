...
prefix. `crates/happenstance-core/src/store.rs:148-168` specifies laziness, ordering
...
boundary for a multi-item query (`store.rs:245-255`). If item 1's late arrival is
...
to the whole `Query` (`store.rs:165-169`).
...
**Falsifies:** that `store.rs:177-180`'s `# Atomicity` paragraph is a complete
...
**Falsifies:** that `append(&self, ..)` (`store.rs:195-199`) is safe to call
...
hardcodes `ReadOptions::new()` (`store.rs:259`) and derives its boundary from
...
`append(&events, None)` (`store.rs:198`) renders "I asserted nothing because there
...
exactly one `Option<&AppendCondition>` for the whole slice (`store.rs:195-199`), so
...
`append(&[losing, superseded], Some(&guard))` — and `store.rs:177-180` guarantees
...
actually performs. `append` is closed over insertion (`store.rs:195-199`); there is no
...
two-trait design (`store.rs:151-155`, CLAUDE.md constraint 3). A unit test already
...
(`store.rs:146`) carries **no** `Send` or `Sync` bound, so an adapter error holding a
...
`#[trait_variant::make(SendEventStore: Send)]` (`store.rs:113`) yields `Send` and `Send`
...
in the crate (`read_decision_model`, `store.rs:245-255`) exists to produce the
