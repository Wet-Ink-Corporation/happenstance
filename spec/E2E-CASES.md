...
[`README.md`](../references/scenarios/README.md). Every case is traced to the scenario that produced it,
...
After [ADR-0006](../.kb/decisions/0006-bare-name-to-the-typed-layer.md) the bare name
...
prefix. `crates/happenstance-core/src/store.rs:149-169` specifies laziness, ordering
...
boundary for a multi-item query (`store.rs:246-256`). If item 1's late arrival is
...
to the whole `Query` (`store.rs:166-170`).
...
decision ledger names as a live candidate (`RUNBOOK.md:64`). Applied per row,
...
**Falsifies:** that `store.rs:178-181`'s `# Atomicity` paragraph is a complete
...
**Falsifies:** that `append(&self, ..)` (`store.rs:196-200`) is safe to call
...
hardcodes `ReadOptions::new()` (`store.rs:260`) and derives its boundary from
...
`append(&events, None)` (`store.rs:199`) renders "I asserted nothing because there
...
exactly one `Option<&AppendCondition>` for the whole slice (`store.rs:196-200`), so
...
`append(&[losing, superseded], Some(&guard))` — and `store.rs:178-181` guarantees
...
actually performs. `append` is closed over insertion (`store.rs:196-200`); there is no
...
two-trait design (`store.rs:152-156`, CLAUDE.md constraint 3). A unit test already
...
(`store.rs:147`) carries **no** `Send` or `Sync` bound, so an adapter error holding a
...
`#[trait_variant::make(SendEventStore: Send)]` (`store.rs:114`) yields `Send` and `Send`
...
in the crate (`read_decision_model`, `store.rs:246-256`) exists to produce the
...
measured (ADR-0016 §13, D1–D4 in `experiments/wire-format/src/lib.rs`), of four
...
and in `references/evaluation/revised-runway.md`. E2E-01 cannot be written as a rule until the
