...
names. [ADR-0006](../docs/adr/0006-bare-name-to-the-typed-layer.md) decided that
...
[0001:5-12](../docs/adr/0001-async-port-flavours.md),
[0003:5-12](../docs/adr/0003-opaque-payloads.md) and
[0004:5-12](../docs/adr/0004-edition-and-msrv.md) each say they were authored before
...
[ADR-0009](../docs/adr/0009-error-send-sync.md) therefore settles the clause the other
...
(which cannot: `spawn_blocking` panics with no runtime in scope and `read` is not
`async`, so its work must move into `poll_next`) both conformant. Laziness is
...
`happenstance-cloudflare` at phase 9, whichever lands first.]**
...
and [ADR-0008](../docs/adr/0008-one-derivation-for-both-ports.md) closed it by taking
...
both ports at once. [ADR-0008](../docs/adr/0008-one-derivation-for-both-ports.md)
takes the derivation scheme for `EventStore` and `ProjectionStore` together and
records where their consequences diverge;
[ADR-0009](../docs/adr/0009-error-send-sync.md) does the same for the `Error` bound
...
- **`Error: Send + Sync` on either flavour.** Settled, and recorded here because
  ...
  [ADR-0009](../docs/adr/0009-error-send-sync.md) keeps
