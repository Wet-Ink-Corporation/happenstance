...
The port declares itself unfrozen in its own first paragraph — *"behind
`unstable-projection`, and this is why"* (`crates/happenstance-core/src/projection.rs:3-11`).
It used to say *"a port without a conformance suite is a guess"*; the suite now
exists and four adapters clear it, and the header says what keeps the gate on
instead. This section's job is not to remove that marker. It is to state
precisely what would remove it, and to settle enough of the shape that an
adapter can be built against something other than a guess.
...
`for_each_model_rule!` at `crates/happenstance-testkit/src/model.rs:800`, and it
lives beside the single rule it names, `ops_agree_with_the_model` at
`crates/happenstance-testkit/src/model.rs:686`. `event_store_model_conformance!`
