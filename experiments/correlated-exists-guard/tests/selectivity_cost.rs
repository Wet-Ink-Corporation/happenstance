...
use correlated_exists_guard::chain::{Selectivity, Shape, chunks};
use correlated_exists_guard::probe_store;
...
fn the_query_planning_quadratics_at_the_specifications_own_floor() {
    for (label, distinct) in [("distinct-tags", true), ("shared-tags", false)] {
        ...
        let planned = median_us(ROUNDS, || SqliteEventStore::planned_statement_count(&query));
...
fn a_full_width_chunk_is_not_chunked_against_the_parameter_limit_either() {
    ...
    assert_eq!(
        plan.len(),
        1,
        "one chunk, because it is bounded by item count"
    );
