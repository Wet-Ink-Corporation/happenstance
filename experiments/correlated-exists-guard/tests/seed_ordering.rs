...
fn most_selective_first_against_least_selective_first() {
    ...
    };
    {
        let mut connection =
            happenstance_sqlite::connection::open_configured(&path).expect("opening must succeed");
        correlated_exists_guard::seed::seed(&mut connection, store_id, SIZE)
    ...
    let check =
        correlated_exists_guard::seed::verify(&connection, SIZE).expect("verify must succeed");
    assert!(
        check.is_sound(SIZE),
    ...
    // **Two shapes, not one.** The sibling experiment measured this on
    // `chain-as-shipped` alone and found the documented ordering 39-44x
    // *slower*. The question here is narrower and is the reason this crate
    // exists: the correlated `EXISTS` form is predicted to make the seed arm the
    // outer loop, which — if it holds — would make most-selective-first the
    // right policy on the shape that ships rather than only on the aggregate
    // that does not. A shape that reversed the sign of this table would be the
    // finding.
    for shape in [Shape::Chain, Shape::ChainExists] {
        for (label, guard) in [
            (
            ...
            // `shape=` is the first field, and it is not decoration. The first
            // version of this file omitted it and left the reader to infer which
            // rows belonged to which shape from the loop's order — which is
            // provenance by convention, and one reordering away from a table that
            // is silently wrong about the thing it exists to compare.
            println!(
                "SEED-ORDER\tshape={}\tsize={SIZE}\tscenario={label}\tboundary={boundary}\t\
