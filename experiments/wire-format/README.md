...
[`experiments/position-visibility`](../position-visibility/README.md)'s
...
`experiments/` "measurements. reproducible, and not in the gate," and
...
- **Compiled size.** Whether `base64`, `postcard`, or the wire types
  ...
  measured here — matching `experiments/position-visibility`'s own
...
- **Runtime performance of the encode/decode paths themselves.** Every
  ...
  `experiments/position-visibility` is what a throughput measurement in
