...
use busy_timeout_margin_probes::{AppendStrategy, CandidateStore, TagStorage};

/// The busy timeout every arm runs under, in milliseconds.
///
/// Re-exported from the library rather than restated, which is the one edit this
/// file carries against `experiments/append-condition/tests/support/mod.rs`. The
/// number under examination may not have two definitions that could drift: this
/// is the same constant the counting handler enforces its cap with, and the same
/// literal `crates/happenstance-sqlite/src/connection.rs:62` ships.
pub use busy_timeout_margin_probes::BUSY_TIMEOUT_MS;
...
impl<A: AppendStrategy, T: TagStorage> CandidateFixture<A, T> {
    ...
    pub fn new() -> Self {
        ...
        let name = format!(
            "happenstance-busy-margin-{}-{}-{}-{ordinal}.sqlite3",
            std::process::id(),
            A::NAME,
            T::NAME,
        );
