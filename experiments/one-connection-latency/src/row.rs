...
/// # Errors
///
/// Returns [`ReplicaError::StoredTag`] if a stored value no longer
/// validates as a [`Tag`], and [`ReplicaError::CorruptTags`] if the
...
/// naming the suite.
pub(crate) fn decode_tags(raw: &[u8]) -> Result<Tags, ReplicaError> {
    let text = core::str::from_utf8(raw).map_err(|_| ReplicaError::CorruptTags)?;
    text.split(UNIT as char)
        .filter(|part| !part.is_empty())
        .map(|part| Tag::new(part).map_err(ReplicaError::StoredTag))
...
/// # Errors
///
/// Returns the stored-value errors above, [`ReplicaError::Sqlite`] if a
/// column cannot be read, [`ReplicaError::InvalidPosition`] if a stored
/// position is not representable, and
/// [`ReplicaError::UnstampedEvent`] if a row carries no identity.
pub(crate) fn to_event(row: &rusqlite::Row<'_>) -> Result<SequencedEvent, ReplicaError> {
