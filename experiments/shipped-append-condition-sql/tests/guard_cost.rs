...
use shipped_append_condition_sql::{Conditions, ProbeStore, seed};
...
fn spread(samples: &mut [u128]) -> (u128, u128, u128) {
    ...
    (samples[samples.len() / 2], samples[last / 10], samples[last])
}
...
fn the_guard_at_three_log_sizes() {
    ...
    for (size, rounds) in SIZES {
        ...
        for (label, tags, boundary) in SCENARIOS {
            ...
            let mut arms: Vec<&dyn RejectingStore> =
                probes.iter().map(|probe| probe as &dyn RejectingStore).collect();
    ...
    assert_eq!(head.get(), last, "the timed runs must not have moved the log");
