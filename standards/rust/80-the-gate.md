...
`xtask/src/main.rs:1419 (fn wasm_steps() -> Vec)` · `xtask/src/main.rs:1459 (fn steps_named)` ·
...
`xtask/src/main.rs:1516 (let Some(probe) = step.probe)` ·
`xtask/src/main.rs:1550 (fn is_available)`
...
**Evidence.** `xtask/src/main.rs:112 (is not portable to the Windows this repository is developed on)` · `xtask/src/main.rs:558 (because rustdoc does not read)` ·
`xtask/src/main.rs:557 (rather than the ambient)`
...
`xtask/src/main.rs:980 (must rewrite the lock file)` ·
...
`xtask/src/main.rs:1509 (A let-chain, and the first in the workspace)` ·
