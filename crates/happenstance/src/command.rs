...
mod tests {
    ...
    // `call_sites_use_the_const_spelling_of_retry_attempts` moved to
    // `tests/retry_spelling.rs` at the `0.2.0` pass. It reads two of the
    // workspace's example crates, and `include_str!` from a file under `src/`
    // reaches outside this package — which ships, and which no `exclude` can
    // reach because the file is the library. The test is unchanged; only its
    // home is, and its new one is excluded from the `.crate`.
