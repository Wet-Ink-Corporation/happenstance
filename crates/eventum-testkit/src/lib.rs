//! The DCB conformance suite for eventum event store adapters.
//!
...
//!
//! The macro binds on [`EventStore`](eventum_core::EventStore), the flavour
//! with no `Send` bound, so it accepts both kinds of adapter. If your adapter
//! implements [`SendEventStore`](eventum_core::SendEventStore) — as every
