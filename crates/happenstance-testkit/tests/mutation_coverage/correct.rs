...
pub(crate) type Allocate = fn(Option<SequencePosition>) -> SequencePosition;
...
pub(crate) fn dense(previous: Option<SequencePosition>) -> SequencePosition {
    match previous {
...
pub(crate) enum LogError {
    ...
    /// [`Defect::select`](crate::mutants::Defect::select) returns a `Result` at
...
pub(crate) fn matching<'a>(events: &'a [SequencedEvent], query: &Query) -> Vec<&'a SequencedEvent> {
    events
        .iter()
...
pub(crate) fn ordered(matched: Vec<&SequencedEvent>, options: ReadOptions) -> Vec<&SequencedEvent> {
...
pub(crate) fn truncated(
    mut selected: Vec<&SequencedEvent>,
    options: ReadOptions,
) -> Vec<&SequencedEvent> {
...
/// `AssertUnwindSafe`.
pub(crate) type Probe = fn(&[SequencedEvent], &AppendCondition) -> Option<SequencePosition>;

/// Position assignment for a batch, as a value. See [`sequence`].
pub(crate) type Sequencer = fn(&[Event], Option<SequencePosition>, Allocate) -> Vec<SequencedEvent>;
...
impl Log {
    ...
    pub(crate) fn new(allocate: Allocate) -> Self {
        Self {
            events: Vec::new(),
            allocate,
        }
...
#[derive(Debug, Clone)]
pub(crate) struct LogStore(Rc<RefCell<Log>>);
...
impl LogStore {
    ...
    pub(crate) fn over(log: &Rc<RefCell<Log>>) -> Self {
        Self(Rc::clone(log))
    }
}
