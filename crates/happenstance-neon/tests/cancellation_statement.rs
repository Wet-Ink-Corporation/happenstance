...
fn the_crate_states_what_a_dropped_append_does() {
    ...
    for token in ["ES-23", "MAY already have committed", "MUST NOT"] {
        assert!(
...
/// The transport trait carries the obligation the store inherits.
///
/// This crate owns no HTTP client, so what a dropped future does to a request in
/// flight is a property of whatever transport a caller supplied. The store's
/// ES-23 answer is only as good as the instruction given to an implementor, and
/// that instruction has to sit ON the method they implement.
#[test]
fn the_transport_tells_an_implementor_what_it_may_not_claim() {
    let transport = include_str!("../src/transport.rs");
    for token in ["cancellation-safe", "MUST NOT"] {
        assert!(
            transport.contains(token),
            "`SqlTransport` no longer tells an implementor it may not present its \
             transport as cancellation-safe (missing `{token}`). The store's ES-23 \
             answer is inherited from that method, so the instruction has to sit on it"
        );
    }
}
