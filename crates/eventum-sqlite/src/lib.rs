//! SQLite adapters for eventum: an event store and a projection store.
//!
//! # Status: not implemented
//!
//! The crate compiles and its shape is fixed, but every operation is
//! `todo!()`. It is `publish = false` until it passes
//! [`eventum-testkit`](https://docs.rs/eventum-testkit)'s conformance suite —
...
//!   SQLite's JSON1 functions. [`Tags`](eventum_core::Tags) is canonically
...
//! [`EventStore`](eventum_core::EventStore) rather than
//! [`SendEventStore`](eventum_core::SendEventStore) — not a feature of this
//! one.
