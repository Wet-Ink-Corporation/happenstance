//! Translating a [`Query`] into a Postgres `WHERE` clause.
//!
//! One translation, two callers: the read path's `DECLARE … CURSOR` and the
//! append condition's conflict probe. They are the same predicate over the same
...
//! tags" in one operator against one GIN index. The SQLite adapter uses a join
//! table with a correlated `EXISTS` per tag because SQLite has no array type and
//! no GIN; that difference is storage-shaped, not semantic.
//!
...
//! moment a value reaches SQL as text, the next person to add a value type has
//! no interpolation-free example to copy.
...
/// A value bound into a generated predicate.
///
/// An enum rather than `sqlx`'s `Arguments` because the parameters are built by
/// one function and bound by another, and an owned, inspectable list is what
/// lets a test assert on the generated plan without executing it.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum Param {
    /// One event type, bound as `text`.
    EventType(String),
    /// A whole tag set, bound as `text[]` for `@>`.
    Tags(Vec<String>),
}
...
pub(crate) fn predicate(query: &Query, next: &mut usize) -> Predicate {
    ...
    Predicate {
        ...
        // SQL must parenthesise each guard explicitly, because the precedence
        // bug that already threatens the single-boundary form becomes n times
        // more likely here.
...
fn item_sql(item: &QueryItem, next: &mut usize, params: &mut Vec<Param>) -> String {
    ...
    if !item.types().is_empty() {
        let placeholders: Vec<String> = item
            ...
            .iter()
            .map(|event_type| {
                params.push(Param::EventType(event_type.as_str().to_owned()));
    ...
    if !item.tags().is_empty() {
        ...
        // `<@` would be the other direction and would silently turn a
        // superset match into an exact-subset one.
    ...
    if conjuncts.is_empty() {
        ...
        // `Query::All` and has nowhere else to come from. Same
        // `#[non_exhaustive]` justification for keeping it; same test below.
...
mod tests {
    ...
    use super::{Param, predicate};

    fn build(query: &Query) -> (String, Vec<Param>) {
        let mut next = 1;
        let built = predicate(query, &mut next);
        (built.sql().to_owned(), built.params().to_vec())
    }
    ...
    ///
    /// `Query::from_items([])` is rejected as `InvalidQuery::NoItems`, so a
    /// query matching nothing cannot be built. The branch stays because `Query`
    /// is `#[non_exhaustive]` and `items()` hands back a plain slice — a future
    /// variant could yield an empty one — and because the failure it guards is
    /// silent and expensive in the wrong direction: emitting `TRUE` for an empty
    /// disjunction turns a query that should match nothing into one that matches
    /// the entire log, and as an append *condition* into one that refuses every
    /// append.
    #[test]
    ...
    fn types_within_an_item_are_or() {
        ...
        assert_eq!(
            params,
            vec![
                Param::EventType("A".to_owned()),
                Param::EventType("B".to_owned())
            ]
        );
    }
    ...
    fn placeholder_numbering_continues_from_the_callers_index() {
        ...
        let mut next = 7;
        let built = predicate(&query, &mut next);
        assert_eq!(built.sql(), "((event_type IN ($7)))");
        assert_eq!(next, 8, "the caller must be told what it may use next");
    }

    /// The `(TRUE)` branch in `item_sql` is likewise unreachable, and for a
    /// stronger reason than the one above.
    ///
    /// `QueryItem` rejects an item with neither types nor tags outright, as
    /// `InvalidQuery::UnconstrainedItem` — so "matches everything" is spelled
    /// `Query::All` and has nowhere else to come from. Kept as a guard for the
    /// same `#[non_exhaustive]` reason, and asserted here so that a reader who
    /// notices the untested branch finds out why it is untestable instead of
