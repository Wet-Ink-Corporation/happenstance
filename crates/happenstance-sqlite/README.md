...
`happenstance_sqlite::rusqlite::…`, and `happenstance_sqlite::tokio::…` for the
`JoinError` and `TryCurrentError` in the error enums. What that buys is *type
identity* — the `rusqlite::Error` you match on is the one this crate's error
enum actually carries, rather than a second copy that prints the same and meets
you as `error[E0308]` the first time you try to act on a failure. Add a
`rusqlite` line of your own when you use SQLite for tables of your own; the
re-export is not a substitute for a dependency, and it forwards only the
features this crate enabled.
