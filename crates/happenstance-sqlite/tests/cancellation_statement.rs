...
///
/// `append` locks the connection and calls a synchronous `rusqlite` path inline;
/// there is no suspension point in it at all, so a future polled once has already
/// run to completion and dropping it cannot cancel anything. That is what the
/// section claims, and this is the only thing in the tree that would notice it
/// stopping being true.
///
/// **It stays true across the move this crate is most likely to make.** ADR-0012
/// and the port's own doc both name a pooled `rusqlite` adapter working in
/// `spawn_blocking` as *"the shape that looks cancellation-safe and is not"*: a
/// dropped `JoinHandle` does not cancel the closure, so the answer would not
/// change — but the `.await` would appear here, this test would go red, and
/// whoever made the change would have to re-read the statement rather than
/// inherit it.
#[test]
fn the_append_body_suspends_nowhere() {
    ...
    assert!(
        ...
        "`append` now contains a suspension point, so the `# Cancellation` section \
         in this crate's root documentation is false as written. Either remove the \
         `.await` or rewrite the statement — and note that moving the body into \
         `spawn_blocking` does not change the answer, because a dropped \
         `JoinHandle` does not cancel the closure"
    );
}
...
/// the crate rather than a shortcut: `crates/happenstance-sqlite/src/event_store.rs`
/// contains **no** `.await` anywhere — the read path hops onto `spawn_blocking`
/// from inside a hand-written `poll_next`, not from an `async fn` — so there is
/// nothing in this file for a real-source control to find.
#[test]
