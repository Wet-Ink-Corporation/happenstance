//! The fixture two test targets share, and nothing else.
//!
//! # Why this is `tests/support/mod.rs` and not `tests/support.rs`
//!
//! Cargo compiles **every file directly under `tests/`** as its own test
//! binary. A `tests/support.rs` holding this fixture would therefore become a
//! third test target containing no `#[test]` — a binary that builds `rusqlite`,
//! links, runs nothing and reports success. Under `tests/support/`, the file is
//! a plain module and each target that wants it writes `mod support;`.
//!
//! Two targets want it, and they are separate on purpose:
//!
//! * `tests/conformance.rs` — the sequential bar,
//!   `event_store_conformance!(SqliteFixture::new())`.
//! * `tests/concurrency.rs` — the racing family, which is opt-in, carries a
//!   different bound (`F::Store: EventStore + Send`) and does not exist on
//!   `wasm32` at all.
//!
...
//!
//! The tokio harness, so: native only. Both including targets carry the `cfg`
//! on their `mod support;` rather than this file carrying an inner one, which
//! keeps each target's own gate readable at the top of that target.
...
#[derive(Debug)]
pub(crate) struct SqliteFixture {
    path: PathBuf,
