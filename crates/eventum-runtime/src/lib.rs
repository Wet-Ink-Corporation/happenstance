//! The typed layer that sits above the [`eventum-core`] contract.
//!
//! # Status: not implemented
//!
//! This crate is a **named seam**, not working code. It exists so that the
//! boundary is visible in the workspace and so that `eventum-core` never
...
//! # What belongs here
//!
//! `eventum-core` deals in opaque bytes on purpose — that is what keeps
...
//!   [`EventType`](eventum_core::EventType) and its
//!   [`Tags`](eventum_core::Tags). A derive macro in a future
//!   `eventum-macros` crate would generate the boilerplate; that crate is
...
//!   [`Query`](eventum_core::Query). Composing several of these into one query
...
//!   [`AppendError::ConditionViolated`](eventum_core::AppendError::ConditionViolated).
//! * **Projection runner** — pumps events from an
//!   [`EventStore`](eventum_core::EventStore) into a
//!   [`ProjectionStore`](eventum_core::ProjectionStore), honouring the
//!   checkpoint invariant described in that trait's module documentation.
//!
//! [`eventum-core`]: eventum_core
