//! Replication between eventum instances.
//!
...
//! eventum instances; neither is privileged in the protocol, though a
//! deployment may well designate one as authoritative.
//!
//! # Why this is a thin crate and not a hard one
//!
//! Because [`Event`](eventum_core::Event) payloads are opaque bytes, a peer
...
//! `eventum-core` free of `serde` in its default feature set, and it is why
//! this crate depends on `eventum-core/serde` explicitly: the envelope is
//! serialised, the payload is passed through.
//!
//! # The hard part, stated honestly
//!
//! [`SequencePosition`](eventum_core::SequencePosition) is meaningful only
...
//!   [`AppendCondition`](eventum_core::AppendCondition) checked against the
...
//!   [`EventStore`](eventum_core::EventStore) — the flavour with no `Send`
//!   bound — not [`SendEventStore`](eventum_core::SendEventStore).
//!
