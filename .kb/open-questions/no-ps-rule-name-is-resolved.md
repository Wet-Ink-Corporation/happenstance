...
excluding the `PS` family outright, at `spec_trace.rs:695-697`. Verified
against this worktree, that half of the finding is now wrong: `has_suite`
(`xtask/src/spec_trace.rs:1750`) admits the `PS-` prefix as of the
2026-08-15 wave, a change held by its own test
(`xtask/src/spec_trace.rs:2376-2380`, asserting `has_suite("PS-1")` and
