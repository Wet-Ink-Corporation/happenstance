...
`crates/happenstance-sqlite/src/event_store.rs:836-881` is `for guard in
...
- A third fact bearing on the falsifier's first limb: the one adapter that *is*
  ...
  statement. `crates/happenstance-sqlite/src/event_store.rs:780` — "**One `SELECT
...
6. **Whether the pin should ever become a conformance rule.** It should not — an
   ...
   (`crates/happenstance-sqlite/src/event_store.rs:836-881`) — but nothing written
