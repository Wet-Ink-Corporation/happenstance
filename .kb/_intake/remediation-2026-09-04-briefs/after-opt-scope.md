...
`crates/happenstance-sqlite/src/event_store.rs:642-675` is `for guard in
...
- A third fact bearing on the falsifier's first limb: the one adapter that *is*
  ...
  statement. `crates/happenstance-sqlite/src/event_store.rs:610` — "**One `SELECT
...
6. **Whether the pin should ever become a conformance rule.** It should not — an
   ...
   (`crates/happenstance-sqlite/src/event_store.rs:642-675`) — but nothing written
