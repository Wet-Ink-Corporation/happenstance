...
`crates/happenstance-sqlite/src/event_store.rs:901-922` — `write_batch`, the multi-row insert the falsifier names:
...
`crates/happenstance-cloudflare/src/event_store.rs:699-728` — `write_rows`, one `INSERT … RETURNING position` per event:
