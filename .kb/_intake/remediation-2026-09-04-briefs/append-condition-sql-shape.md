...
`crates/happenstance-sqlite/src/event_store.rs:804-818`:
...
The table behind the sort is maintained on every write, inside the transaction (`event_store.rs:961-971`):
...
- *"It changes one string in a private module and touches no correctness surface"* — half false. `chunks` is reached from the guard path under `BEGIN IMMEDIATE` (`event_store.rs:794`) and from the read path (`:1313`), and B **adds** a `tags.len() == 1` branch that shipped `item_sql` does not have (`query_sql.rs:347-385` against `chain.rs:441-443`).
...
- Two smaller defects in the same critique, both understating B's cost, were folded into option B: `chunks` is shared by the guard path (`event_store.rs:794`) and the read path (`:1313`), so B is not "one string in a private module"; and shipped `item_sql` has no `tags.len() == 1` branch, so B adds one rather than keeping one.
