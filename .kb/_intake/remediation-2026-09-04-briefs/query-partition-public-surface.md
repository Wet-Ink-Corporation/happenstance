...
Something like `pub const fn query_ceilings() -> (usize, usize)`, or a small `QueryCeilings` struct on the model of `happenstance-cloudflare`'s `Ceilings` (`crates/happenstance-cloudflare/src/event_store.rs:189-196`).
