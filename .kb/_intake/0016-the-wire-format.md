...
**[`experiments/wire-format/`](../../experiments/wire-format/README.md)**, outside
...
`references/evaluation/research-dcb-spec.md`; what `JSON.stringify` would literally emit for
...
GeoJSON in `Event::data`"* (`references/scenarios/README.md:1367`), read as 340 KiB = 348,160
...
`experiments/wire-format/no-std-wasm-check/` is a `#![no_std]` + `alloc` crate whose
...
`experiments/wire-format/src/lib.rs`, against `happenstance-core` at HEAD with
...
- ~~**Whether a `no_std` base64 build actually exists.**~~ **Closed at phase 5, by
  measurement** — `experiments/wire-format/no-std-wasm-check/` checks clean on
...
- **`:2110-2113`** (*"340 KB seat map becomes roughly 1.3 MB of JSON"*) **becomes**: *"so
  Turnstile's seat map (`references/scenarios/README.md:1367`), taken as 340 KiB = 348,160 bytes,
...
- **`experiments/wire-format/src/lib.rs`** — add the `Deserialize` half of `Detect<T>`
