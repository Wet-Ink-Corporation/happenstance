...
compiler fails the gate, so the reproduction lives in `docs/experiments/`.
...
`docs/experiments/rustc-ice-gat-foreign-trait/bisect.sh`; the recorded answer is
...
`docs/adapter-shapes.md:346 (Five ingredients)` ·
[SPECIFICATION PS-5](../architecture/SPECIFICATION.md) ·
[adapter-shapes §6](../adapter-shapes.md) ·
...
**Evidence.** `docs/adapter-shapes.md:325 (rustc crashes **while diagnosing a region error**)` ·
...
**Evidence.** `docs/evaluation/research-rust-api-guidelines.md:47 (Do not design around it)` ·
`crates/happenstance-core/src/store.rs:93 (trait_variant::make)` ·
[SPECIFICATION ES-1](../architecture/SPECIFICATION.md) ·
