...
[SPECIFICATION PS-36](../architecture/SPECIFICATION.md) *(`[FROZEN]`: the `Send`
flavour transitively requires `Batch: Send`, and why no gate can pin it)* ·
[SPECIFICATION ES-5](../architecture/SPECIFICATION.md) *(why the bound cannot be
...
[SPECIFICATION ES-6](../architecture/SPECIFICATION.md) *(`[FROZEN]`: the
obligation, and why it is asserted on the `Output`)* ·
[ADR-0009](../adr/0009-error-send-sync.md)
...
[SPECIFICATION ES-5](../architecture/SPECIFICATION.md) *(`[FROZEN]`: bounds are
identical on both flavours by construction)* ·
[SPECIFICATION ES-6](../architecture/SPECIFICATION.md) *(`[FROZEN]`: the bound
