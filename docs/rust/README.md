...
> `CONTRIBUTING.md` summary > `docs/evaluation/*`.**
...
- **Clause or atom?** Ask: *could a conformant adapter written in another
  ...
  [`SPECIFICATION.md`](../architecture/SPECIFICATION.md), and the atom cites it.
  No → it is a Rust idiom, and the atom owns it. An atom never restates a
  clause's content.
- **`docs/evaluation/*` is dated evidence, not doctrine.** It is pinned to a
  commit and may be stale. Worked example: its
  [Rust API audit](../evaluation/research-rust-api-guidelines.md) §16 declares
...
| [`01-standard-of-evidence.md`](01-standard-of-evidence.md) | adding or editing a rule in `docs/rust/` · reaching for a | RS-01-1, RS-01-2, RS-01-3, RS-01-4 |
...
[`SPECIFICATION.md` §1.3](../architecture/SPECIFICATION.md) on purpose, so the
two do not fork.
