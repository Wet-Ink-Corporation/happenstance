...
[ES-10](../../architecture/SPECIFICATION.md) by construction: a writer takes 99, a
...
```console
$ bash docs/experiments/position-visibility/run.sh
```
...
```console
$ bash docs/experiments/position-visibility/run.sh inversion    # §3, ~1 minute
$ bash docs/experiments/position-visibility/run.sh throughput   # §4, ~30 minutes
$ bash docs/experiments/position-visibility/run.sh staleness    # §5, ~4 minutes
$ bash docs/experiments/position-visibility/run.sh keep         # everything, leave the container up
```
