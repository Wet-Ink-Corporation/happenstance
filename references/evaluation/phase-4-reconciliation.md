...
stream and is capped at 64 MiB — is documented at `references/adapter-shapes.md:214-215`.
...
The evidence is `experiments/position-visibility/`, run against a real
...
- **That any CF-25 far end is filled.** Zero adapter instruments across all seven
  ...
  already `[FROZEN]`.
- **That a 128-bit `StoreId` crosses wasm32.** `references/adapter-shapes.md:220`
...
VT-17's missing E2E case lands in `spec/E2E-CASES.md`, not in
`SPECIFICATION.md`.
