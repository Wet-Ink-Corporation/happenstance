...
> **Status: `0.3.1`, and worth being precise about what that number claims.** The
...
> in the `0.3.1` release set**, which is the set the manifests and
> `xtask/src/package.rs`'s `PUBLISHABLE` agree on.
>
> What `0.3.1` promises, as `0.3.0` did: every clause marked `[FROZEN]` in
...
A ✅ below means **in the `0.3.1` release set and passing its suite**. It does not
say the version is on the registry: nothing is published at `0.3.1` until the
publish sequence has actually been run, and this table is written before it.

| Crate | Role | Status |
|---|---|---|
| [`happenstance`](crates/happenstance) | Codecs, typed domain events, decision models — the crate an application programs against | ✅ in the `0.3.1` release set |
| [`happenstance-core`](crates/happenstance-core) | DCB types, storage ports, in-memory reference store | ✅ in the `0.3.1` release set |
| [`happenstance-testkit`](crates/happenstance-testkit) | Conformance suite adapters must pass | ✅ in the `0.3.1` release set — 116 rules across four families |
| [`happenstance-sqlite`](crates/happenstance-sqlite) | SQLite event store and projection store | ✅ in the `0.3.1` release set; passes the suite against a real file |
| [`happenstance-cloudflare`](crates/happenstance-cloudflare) | Durable Object event store — the workspace's only `!Send` store, and the reason the ports have two flavours | ✅ in the `0.3.1` release set; passes the suite on `wasm32` — read the note below |
| [`happenstance-postgres`](crates/happenstance-postgres) | Postgres event store and projection store — the target that does *not* serialise its writers | ✅ in the `0.3.1` release set; both roles pass their suites against a live server, and the first adapter to clear the concurrency family against a store that does not serialise its writers |
| [`happenstance-neon`](crates/happenstance-neon) | Postgres over one-shot HTTP: no connection, no interactive transaction, no cursor | ✅ in the `0.3.1` release set; passes its suites against a live endpoint, with one open clause question its README names |
...
The one 🔲 row is not a placeholder in the empty sense: it carries the design
...
suite, 14 rules run and 3 reported as skips. **It ships in `0.3.1`**, together
...
passing its suite*; nothing is in the `0.3.1` release set until the publish
...
`0.3.1` with the thinnest evidence of the seven. Two things a reader should have
...
**This README makes no performance claim, and that is deliberate.** The library
is positioned on correctness, and an earlier version of that positioning opened
with the words "fast, efficient" while nothing in the repository supported them —
three of the fourteen reviews say so. The numbers came out on purpose.

What exists instead is a measured record, kept where it can be checked:

* [`benchmarks/results/GRADES.md`](benchmarks/results/GRADES.md) — what the
  abstraction costs over raw SQL, throughput, the DCB guard, the typed layer,
  the projection runner, and exact allocation counts. Every figure is written by
  hand from `benchmarks/results/raw/`, which is committed beside it.
* [`benchmarks/README.md`](benchmarks/README.md) — the conditions each figure was
  taken under, and a **What none of this shows** section that is the first thing
  to read if you are about to quote one.

Two of its own rules are worth repeating here, because they are what make the
rest usable. Read the figures as **orders of magnitude and as ratios between arms
of one run**; nothing there should be quoted to a third significant figure. And
prefer the allocation counts to the timings — they are reproducible to the digit
where the wall clock is not.

`benchmarks/` is not a workspace member, appears in no `cargo xtask ci` step, and
contains no threshold on any budget. CF-34 forbids a benchmark result gating a
merge: an adapter that is slow is conformant. To re-derive the whole record from
a clean checkout, `cd benchmarks && ./run.sh`.
